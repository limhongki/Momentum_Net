...
data_name = 'phantom_momentum_scnn_r_7';
...
for kp = 1:num
    ...
    output = py.mypcodes.train_scnn.train(int32(kp-1),...
        pyargs('kern_size',int32(sqrt(R)), 'kern_num',int32(K), 'pad_size',int32(Rpad),...
        'lr_enc',lr_enc, 'lr_dec',lr_dec, 'lr_threshold',lr_threshold,...
        'num_epoch',int32(num_epoch), 'Lbatch',int32(Lbatch), 'optim', optim,...
        'data_name', data_name, 'which_gpu', which_gpu));
    ...
    for i = 1:size(Irecon,3)
        filename = ['IMout_',data_name,'_Layer',num2str(kp),'_image_',num2str(i-1),'.mat'];
        load(['./mypcodes/cache/',filename])
        IMout_ls(:,:,i) = IMout;
    end
    
    for i = 1:size(Irecon_test,3)
        filename = ['IMout_test_',data_name,'_Layer',num2str(kp),'_image_',num2str(i-1),'.mat'];
        load(['./mypcodes/cache/',filename])
        IMout_ls_test(:,:,i) = IMout_test;
    end
    
    clear IMout IMout_test
    IMout = IMout_ls;
    IMout_test = IMout_ls_test; 
    clear IMout_ls IMout_ls_test
...
%% Save and display
if resnet == 1
    name = ['Momentum_v0,scnn_Delta(resnet,rho',num2str(rho), ')_optim_', optim, '_R_', num2str(R), '_K_',num2str(K), '_cn_', num2str(cn), '_layer_',num2str(num), '_epoch_',num2str(num_epoch), '_batch_',num2str(Lbatch), '_lr_',num2str(lr_dec),'_',num2str(lr_enc),'_',num2str(lr_threshold),'.mat'];
else
    name = ['Momentum_v0,scnn_Delta(rho',num2str(rho), ')_optim_', optim, '_R_', num2str(R), '_K_',num2str(K), '_cn_', num2str(cn), '_layer_',num2str(num), '_epoch_',num2str(num_epoch),'_batch_',num2str(Lbatch), '_lr_',num2str(lr_dec),'_',num2str(lr_enc),'_',num2str(lr_threshold),'.mat'];
end
